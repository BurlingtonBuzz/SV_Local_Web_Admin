...
angular.module('inspinia')
    .controller('helpcenterCreateCtrl', ['$scope', '$state', 'NgMap', 'toaster', '$timeout', '$stateParams', '$rootScope', '$uibModal', '$log', 'FileUploader', 'configurationService', '$controller', 'bragService', function ($scope, $state, NgMap, toaster, $timeout, $stateParams, $rootScope, $uibModal, $log, FileUploader, configurationService, $controller, bragService) {
        ...
        $scope.editMember = function () {
            if ($state.current.breadcrumb.text == "Edit") {
                bragService.getMember($stateParams.id).then(function (data) {
                    $scope.member = data;
        ...
        $scope.save = function (model) {
            ...
            if (error == 0) {
                if ($state.current.breadcrumb.text == "Create") {
                    dealService.createMember($scope.member).then(function (data) {
                        $state.go('members');
                        toaster.pop({
                            type: 'success',
                    ...
                    dealService.updateMember($scope.member).then(function (data) {
                        $state.go('members');
                        toaster.pop({
                            type: 'success',
