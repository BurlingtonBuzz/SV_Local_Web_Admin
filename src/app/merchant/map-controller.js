...
angular.module('inspinia')
    ...
    .controller('regionMapController', ['$scope', '$state', 'NgMap',  'toaster', 'FileUploader', 'configurationService', '$stateParams', 'regionService', '$timeout', 'GeoCoder', '$uibModal', '$rootScope', '$uibModalInstance', '$log', 'cityService', function ($scope, $state, NgMap,  toaster, FileUploader, configurationService, $stateParams, regionService, $timeout, GeoCoder, $uibModal, $rootScope, $uibModalInstance, $log, cityService) {

        var regionMap;
        var polygonInstance;
        var polygonData;
        $scope.startpoint = 'Burlington';
        ...
        NgMap.getMap('regionMap').then(function (map) {
            ...
            google.maps.event.addListenerOnce(map, 'idle', function () {
                ...
                $scope.onMapOverlayCompleted = function (e) {
                    console.log(e);
                    polygonInstance = e.overlay;
                    $scope.post.geoFencing = [];
                    console.log(e.overlay.getPath().getArray());
                    $scope.drawing_control_options.drawingModes = [];
                    angular.forEach(e.overlay.getPath().getArray(), function(val, key) {
                        $scope.post.geoFencing.push({ lat: e.overlay.getPath().getArray()[key].lat(), lng: e.overlay.getPath().getArray()[key].lng() });
                    });
